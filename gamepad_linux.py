...
class Gamepad_linux:
    ...
    def convertState(self):
        res = state()
        res.Gamepad = Gamepad()
        buttons = self.pressedMap
        axes = self.axisMap
        buttonsConverted = 0x0
        if buttons[0]:
            buttonsConverted += 0x1000
        elif buttons[1]:
            buttonsConverted += 0x2000
        elif buttons[2]:
            buttonsConverted += 0x4000
        elif buttons[3]:
            buttonsConverted += 0x8000
        elif buttons[4]:
            buttonsConverted += 0x0100
        elif buttons[5]:
            buttonsConverted += 0x0200
        elif buttons[6]:
            buttonsConverted += 0x0020
        elif buttons[7]:
            buttonsConverted += 0x0010
        elif buttons[8]:
            buttonsConverted += 0x0
        elif buttons[9]:
            buttonsConverted += 0x0040
        elif buttons[10]:
            buttonsConverted += 0x0080
        elif axes[6] == -1.0:
            buttonsConverted += 0x4
        elif axes[6] == 1.0:
            buttonsConverted += 0x8
        elif axes[7] == -1.0:
            buttonsConverted += 0x1
        elif axes[7] == 1.0:
            buttonsConverted += 0x2
        res.Gamepad.wButtons = buttonsConverted
        res.Gamepad.bLeftTrigger = (int)((axes[2] + 1) * 127)
        res.Gamepad.bRightTrigger = (int)((axes[5] + 1) * 127)
        res.Gamepad.sThumbLX = (int)(axes[0] * 32767)
        res.Gamepad.sThumbLY = (int)(axes[1] * 32767)
        res.Gamepad.sThumbRX = (int)(axes[3] * 32767)
        res.Gamepad.sThumbRY = (int)(axes[4] * 32767)
        return res
    ...
    class UpdateThread(threading.Thread):
        ...
        def __init__(self, gamepad):
            ...
            if isinstance(gamepad, Gamepad_linux):
    ...
    def _rawEventToDescription(self, event):
        ...
        if eventType == Gamepad_linux.EVENT_CODE_BUTTON:
        ...
        elif eventType == Gamepad_linux.EVENT_CODE_AXIS:
            if index in self.axisNames:
                axis = self.axisNames[index]
            else:
                axis = str(index)
            position = value / Gamepad_linux.MAX_AXIS
            return '%010u: Axis %s at %+06.1f %%' % (timestamp, axis, position * 100)
        elif eventType == Gamepad_linux.EVENT_CODE_INIT_BUTTON:
            if index in self.buttonNames:
                button = self.buttonNames[index]
            else:
                button = str(index)
        ...
        elif eventType == Gamepad_linux.EVENT_CODE_INIT_AXIS:
            if index in self.axisNames:
                axis = self.axisNames[index]
            else:
                axis = str(index)
    ...
    def getNextEvent(self, skipInit = True):
        ...
        finalValue = None
        if eventType == Gamepad_linux.EVENT_CODE_BUTTON:
            eventName = Gamepad_linux.EVENT_BUTTON
            if index in self.buttonNames:
            ...
            eventName = Gamepad_linux.EVENT_AXIS
            if index in self.axisNames:
                entityName = self.axisNames[index]
            else:
                entityName = index
            finalValue = value / Gamepad_linux.MAX_AXIS
    ...
    def startBackgroundUpdates(self, waitForReady = True):
        ...
        self.updateThread = Gamepad_linux.UpdateThread(self)
