...
def inputHandlingPAD(state):
    ...
    time.sleep(1)  # задержка в секундах min = 0.5
...
def inputHandlingUI():
    while True:
        ...
        if state:
            ...
            ui.checkBox_Y_btn.setEnabled(True)
            ui.checkBox_A_btn.setEnabled(True)
            ui.checkBox_LB_btn.setEnabled(True)
            ui.checkBox_RB_btn.setEnabled(True)
            ui.verticalSlider_RT.setEnabled(True)
            ui.verticalSlider_LT.setEnabled(True)
            ...
            inputHandlingPAD(state)
        elif state is None:
            ui.label_padNotDetected.setText("!!! Геймпад не обнаружен !!!")
            ui.checkBox_Y_btn.setEnabled(False)
            ui.checkBox_A_btn.setEnabled(False)
            ui.checkBox_LB_btn.setEnabled(False)
            ui.checkBox_RB_btn.setEnabled(False)
            ui.verticalSlider_RT.setEnabled(False)
            ui.verticalSlider_LT.setEnabled(False)
            inputHandlingPAD(state)
...
if __name__ == "__main__":
    ...
    MainWindow.show()
    inputStream = Thread(target=inputHandlingUI, args=(), daemon=True)
    inputStream.start()
    sys.exit(app.exec())
